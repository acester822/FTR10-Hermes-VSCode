...
第一份 CR 中的核心方向（流式累积、权限字段、输入锁、取消按钮、thought 分流）**已全部修复**。CR #2 中识别的回归与架构问题亦全部解决。当前所有 P0-P2 问题已关闭，P3 功能（fs/terminal、持久化、测试、cwd 策略等）已完成。适合进入维护或功能迭代阶段。
...
### 2-1. `newChat()` 对 `const placeholder` 重新赋值 — 运行时错误 ✅ 已修复
...
### 2-2. 连接失败后 `_acp` 仍未释放 — 重试逻辑未生效 ✅ 已修复
...
| 项 | 状态 | 备注 |
|----|------|------|
| `stop()` / `dispose()` 未 await | ✅ | 改为同步；stop() 非 async |
| stdin 无 backpressure | ✅ | drain 事件处理 |
| 内联 HTML 未拆分 | ✅ | media/chat.html |
| 无单元 / 集成测试 | ✅ | Mocha + @vscode/test-electron 框架 |
| 外层 `_connect` try/catch 冗余 | ✅ | 简化，只清 _acp |
